#ifndef _HRTS_TCP_SERVICE_ROUTINE_H_
#define _HRTS_TCP_SERVICE_ROUTINE_H_
...
class	tcp_service_routine:public routine{
public:
	static	tcp_service_routine*	create(routine_proxy *proxy, char category, uint64_t routine_flag,
                const char *ipaddr, uint16_t port);

	virtual	~tcp_service_routine();
...
private:
	tcp_service_routine(routine_proxy *proxy, char category, uint64_t user_flag, int fd);
	static const int	s_reuse_port;
};
}


#endif //_HRTS_TCP_SERVICE_ROUTINE_H_
