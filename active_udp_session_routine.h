#ifndef _HRTS_ACTIVE_UDP_SESSION_ROUTINE_H_
#define _HRTS_ACTIVE_UDP_SESSION_ROUTINE_H_
...
class	active_udp_session_routine:public routine{
public:
	static	active_udp_session_routine*	create(routine_proxy *proxy, int category, uint64_t user_flag,
		const char *ipaddr, uint16_t port);

	virtual ~active_udp_session_routine();
...
private:
	int     	dispose(const char *data, const unsigned len);
	...
	active_udp_session_routine(routine_proxy *proxy, char category, uint64_t user_flag, int fd);
	static	void	set_flags(routine_proxy *proxy, int fd);
};//end of class active_udp_session_routine
}//end of namespace hrts


#endif //_HRTS_ACTIVE_UDP_SESSION_ROUTINE_H_
